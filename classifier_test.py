...
def format_display():
    fig.canvas.setWindowTitle('Epoch {}'.format(epoch))
    fig.clear()
    plt.xlim(x_limits)
    plt.ylim(y_limits)
    axes = plt.gca()
...
def display(session, loss_values, points, labels, out_val, done):
    ...
    for i in range(len(points)):
        correct = np.argmax(out_val[i]) == list(labels)[i]
        color = marker_colors[list(labels)[i]]
        plt.scatter(transform_x(list(points)[i][0]), transform_y(list(points)[i][1]), color=color, s=60, edgecolors=('black' if correct else 'white'), linewidth=(1 if correct else 2))
